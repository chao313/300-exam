...
export default {
  ...
  methods: {
    edit(examCode) { //编辑试卷
      ...
      this.$axios(`/300-exam/exam/${examCode}`).then(res => { //根据试卷id请求后台
    ...
    getExamInfo() { //分页查询所有试卷信息
      this.$axios(`/300-exam/exams/${this.pagination.current}/${this.pagination.size}`).then(res => {
